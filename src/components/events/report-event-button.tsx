...
import { reportEventListing } from "@/lib/actions/event-listings";
...
export function ReportEventButton({ eventListingId }: { eventListingId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await reportEventListing(eventListingId, reason, detail);
      setDone(true);
    } finally {
      setSubmitting(false);
    }
    ...
    <>
      ...
      {open && (
        <form
          ...
          className="mt-3 space-y-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-4"
        >
          <p className="text-xs font-semibold text-gray-700 dark:text-gray-300">Why are you reporting this listing?</p>
