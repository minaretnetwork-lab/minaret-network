...
- A single local production environment with public access through Cloudflare Tunnel
...
Current local production environment:
...
- Google OAuth credentials configured for the production callback URL
- Optional OpenAI API key for the AI match assistant

## Environment setup

The repo includes scripts for setting up and running the production environment.
...
1. Make changes on a Git branch.
2. Sync the production database schema if the Prisma schema changed.
3. Build production into `.next-production`.
4. Restart the production local server.
5. Smoke-check public and protected routes.
...
Before manual Prisma work, set `DATABASE_URL` and `DIRECT_URL` for production:
